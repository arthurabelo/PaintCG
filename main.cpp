    ...
    if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN){
        if(currentTool == TOOL_LINE){
            if(!click1){ click1=true; x1c=x; y1c=yy; }
                ...
                // draw and store
                shapes.push_back(std::make_shared<LineShape>(IPoint{x1c,y1c}, IPoint{x,y1c}));
                shapes.back()->draw(Color(0,0,0,255));
                click1=false; tempPoly.clear();
            }
        } else if(currentTool == TOOL_POLY){
            tempPoly.emplace_back(x,yy);
            if(tempPoly.size()>=4){ // allow finish with right-click or double-click; here keep building
            }
